...
function abstract_apply(interp::AbstractInterpreter, argtypes::Vector{Any}, sv::InferenceState,
    ...
    is_overdubbing_call(argtypes) && popfirst!(argtypes) # TODO handle me
...
function abstract_invoke(interp::AbstractInterpreter, argtypes::Vector{Any}, sv::InferenceState)
    ...
    if isoverdub
        ...
        shadow_sig = Tuple{ctx, overdub.spec_types.parameters...}
    ...
    #     argtypes′[i] = t ⊑ a ? t : a
    # end
    isoverdub && pushfirst!(ctx, argtypes′)
    const_result = abstract_call_method_with_const_args(interp, result, argtype_to_function(ft′), argtypes′, match, sv, false)
    if const_result !== nothing
        const_rt, const_result = const_result
        if const_rt !== rt && const_rt ⊑ rt
            return CallMeta(const_rt, InvokeCallInfo(unwrap_shadow(match), const_result))
        else
            return CallMeta(rt, InvokeCallInfo(unwrap_shadow(match), nothing))
        end
    end
...
function abstract_call_known(interp::AbstractInterpreter, @nospecialize(f),
    ...
    # COMBAK we want to allow a compiler plugin to modify these functions ?
